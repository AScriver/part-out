...
class PostContainer extends Component {
  render() {
    ...
    return (
      <div
        className="row hover-effect no-gutters pb-0 mb-3"
      >
        ...
        <div className="userpost col-9 d-flex align-content-around flex-column">
          <h3 className='mb-auto text-center'>
            <Link to={`/item/${this.props.post.id}`}>{this.props.post.title} - {this.props.post.category}</Link>
          </h3>
          <hr />
          <div className="row">
          <div className="col-10">
            <p className="my-auto mx-3"><span className="postLabel">Posted By: <Link to={`/user/${this.props.post.User.id}`}>{this.props.post.User.username}</Link></span> - {moment(this.props.post.createdAt).fromNow()}</p>
            ...
            <div className="col-2">
            {(this.props.username === this.props.post.User.username) && (this.props.id === this.props.check)
                    ?
